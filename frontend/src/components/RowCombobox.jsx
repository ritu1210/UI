  ...
  return (
    <>
      <input
        ...
        onBlur={() => setTimeout(() => { setText((t) => (t !== picked ? picked : t)); setOpen(false) }, 150)}
      />
      {open && (
        <div className="combo-pop" style={{ left: pos.left, top: pos.top, width: pos.width }}>
          {items.length === 0 ? (
            <div className="combo-empty">No matches</div>
          ) : (
            items.map((it, i) => (
              <div key={it.value + i} className="combo-opt" onMouseDown={(e) => { e.preventDefault(); choose(it) }}>
                <span className="co-main">{it.label}</span>
                {it.sub && <span className="co-sub">{it.sub}</span>}
              </div>
            ))
          )}
        </div>
